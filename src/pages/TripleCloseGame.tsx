...
export default function TripleCloseGame() {
  ...
  const addToSlip = () => {
    ...
    // Normalize
    let normalized = panelNumber.trim().replace(/\D/g, '');
    
    // Auto-fill leading series to make it exactly 3 digits
    ...
    if (normalized.length !== 3) {
      setError('Please enter a valid 3-digit Panel combination (000 - 999).');
      return;
    }
    ...
    setPanelNumber('');
    setSuccess(`Close Panel "${normalized}" added to betting slip!`);
  };
  ...
  const handleSubmitAll = async (e: React.FormEvent) => {
    ...
    if (betSlip.length === 0) {
      setError('Your betting slip is empty. Add triple predictions first.');
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await placeMultiBets('Triple Close', selectedMarket, betSlip);
      if (res.success) {
        setSuccess(`⚜️ Triple Close Panel entries placed successfully! Commited total of ${totalRequired} coins inside "${selectedMarket}". Best of luck! ⚜️`);
        setBetSlip([]);
      } else {
        setError(res.msg);
      }
  ...
  return (
    <div id="triple-close-arena" className="space-y-6 pb-12 max-w-4xl mx-auto">
      {/* Return button */}
      <button 
        onClick={() => navigate('/')}
      ...
      <div className="bg-gradient-to-r from-zinc-950 via-neutral-900 to-amber-950/10 border-2 border-amber-500/20 p-6 rounded-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 text-xs text-amber-400 font-mono uppercase tracking-widest font-bold mb-1">
          <Trophy className="w-4 h-4 text-amber-500 animate-pulse" /> Supreme Payout Multiplier: 900x
        </div>
        <h2 className="font-serif text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-amber-300 to-amber-500 uppercase tracking-widest leading-none">
          TRIPLE CLOSE ARENA (PANEL)
        </h2>
        <p className="text-neutral-405 text-xs mt-1.5 max-w-2xl leading-relaxed">
          Predict the 3-digit closing combination of the published draw (range <strong className="text-amber-400 font-mono text-sm">000 - 999</strong>). Placing multiple panels inside our slip optimizes coverage. Winning close panel guess pays <strong className="text-amber-400 font-serif">900x your coins</strong>!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-zinc-950 border border-amber-500/25 rounded-2xl shadow-2xl p-5 sm:p-6 space-y-6">
          <h3 className="text-amber-400 text-xs font-mono font-bold tracking-widest uppercase border-b border-amber-500/10 pb-2">
            ⚜️ Create Triple Close Page Slip
          </h3>

          {error && (
            <div className="p-3 bg-red-955/40 border border-red-500/30 text-red-400 text-xs rounded-lg flex items-start gap-2 animate-fadeIn">
          ...
          {success && (
            <div className="p-3 bg-emerald-955/40 border border-emerald-500/30 text-emerald-400 text-xs rounded-lg flex items-start gap-2 animate-fadeIn">
          ...
          <div>
            <label className="block text-[10px] font-mono text-amber-500/80 uppercase tracking-wider mb-2">
              2. Enter Triple Digit Panel (000 - 999)
            </label>
            <input 
              type="text"
              maxLength={3}
              value={panelNumber}
              onChange={(e) => setPanelNumber(e.target.value.replace(/\D/g, ''))}
              placeholder="e.g., 568"
              className="w-full bg-zinc-900 border-2 border-amber-500/20 text-center text-neutral-105 font-serif text-3xl py-3.5 rounded-lg outline-none focus:border-amber-400 font-bold tracking-widest transition-all"
            />
            
            {/* Quick selections list */}
            <div className="grid grid-cols-4 gap-1.5 mt-3">
              {['111', '222', '333', '444', '555', '666', '777', '888'].map((v) => (
                <button
                  key={v}
                  type="button"
                  onClick={() => setPanelNumber(v)}
                  className="py-1 text-[10.5px] font-mono bg-zinc-900 border border-amber-500/10 hover:border-amber-400 text-neutral-400 hover:text-amber-355 rounded cursor-pointer transition-colors animate-fadeIn"
                >
                  {v}
                </button>
              ))}
        ...
        {/* Betting Slip Area */}
        <div className="lg:col-span-5 bg-zinc-950 border border-amber-500/25 rounded-2xl shadow-2xl p-5 sm:p-6 flex flex-col justify-between" id="arena-betting-slip-container-close">
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-amber-500/10 pb-2">
              <h3 className="text-amber-400 text-xs font-mono font-bold tracking-widest uppercase animate-pulse">
                ⚜️ Active Panel Slip
              </h3>
              {betSlip.length > 0 && (
                <button 
                  onClick={clearSlip}
                  className="text-neutral-500 hover:text-red-400 font-mono text-[10px] transition-colors cursor-pointer border-none bg-transparent"
                >
                  Clear Slip
                </button>
              )}
            ...
            {betSlip.length === 0 ? (
              <div className="py-12 text-center text-neutral-600 font-mono text-xs">
                Your slip is empty.<br />Enter a 3-digit Panel and click Add.
              </div>
            ) : (
                ...
                {betSlip.map((item, idx) => (
                  <div key={idx} className="bg-zinc-900/60 border border-amber-500/5 p-2.5 rounded-lg flex items-center justify-between text-xs font-mono animate-fadeIn">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-neutral-500">PANEL:</span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        ...
                        className="text-neutral-600 hover:text-red-400 p-0.5 transition-colors cursor-pointer border-none bg-transparent"
                        title="Remove"
                      >
          ...
          <div className="mt-6 border-t border-amber-500/10 pt-4 space-y-4">
            <div className="space-y-1.5 text-xs font-mono">
              ...
              <div className="flex justify-between text-neutral-400">
                <span>Estimated Win Payout (900x):</span>
                <span className="text-emerald-400 font-bold font-serif">+{(totalCurrentCoins * 900).toLocaleString()} Coins</span>
              </div>
            </div>
            ...
            >
              ...
              {loading ? 'PROCESSING CLIENT SLIP...' : `PLACE ARENA ENTRY [${totalCurrentCoins.toLocaleString()} COINS]`}
            </button>
          </div>
        </div>
      </div>
