...
export default function TripleOpenGame() {
  ...
  const addToSlip = () => {
    ...
    setPanelNumber('');
    setSuccess(`Open Panel "${normalized}" added to betting slip!`);
  };
  ...
  const handleSubmitAll = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await placeMultiBets('Triple Open', selectedMarket, betSlip);
      if (res.success) {
        setSuccess(`⚜️ Triple Open Panel entries placed successfully! Commited total of ${totalRequired} coins inside "${selectedMarket}". Best of luck! ⚜️`);
        setBetSlip([]);
      } else {
        setError(res.msg);
      }
  ...
  return (
    <div id="triple-open-arena" className="space-y-6 pb-12 max-w-4xl mx-auto">
      {/* Return button */}
      <button 
        onClick={() => navigate('/')}
      ...
      <div className="bg-gradient-to-r from-zinc-950 via-neutral-900 to-amber-950/10 border-2 border-amber-500/20 p-6 rounded-2xl relative overflow-hidden">
        ...
        <h2 className="font-serif text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-amber-300 to-amber-500 uppercase tracking-widest leading-none">
          TRIPLE OPEN ARENA (PANEL)
        </h2>
        <p className="text-neutral-405 text-xs mt-1.5 max-w-2xl leading-relaxed">
          Predict the 3-digit opening combination of the published draw (range <strong className="text-amber-400 font-mono text-sm">000 - 999</strong>). Placing multiple panels inside our slip optimizes coverage. Winning panel guess pays <strong className="text-amber-400 font-serif">900x your coins</strong>!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-zinc-950 border border-amber-500/25 rounded-2xl shadow-2xl p-5 sm:p-6 space-y-6">
          <h3 className="text-amber-400 text-xs font-mono font-bold tracking-widest uppercase border-b border-amber-500/10 pb-2">
            ⚜️ Create Triple Open Page Slip
          </h3>
            ...
            <div className="grid grid-cols-4 gap-1.5 mt-3">
              {['111', '222', '333', '444', '555', '666', '777', '888'].map((v) => (
                <button
                  ...
                  className="py-1 text-[10.5px] font-mono bg-zinc-900 border border-amber-500/10 hover:border-amber-400 text-neutral-400 hover:text-amber-350 rounded cursor-pointer transition-colors animate-fadeIn"
                >
                  {v}
                </button>
              ))}
        ...
        {/* Betting Slip Area */}
        <div className="lg:col-span-5 bg-zinc-950 border border-amber-500/25 rounded-2xl shadow-2xl p-5 sm:p-6 flex flex-col justify-between" id="arena-betting-slip-container">
