...
export default function SingleOpenGame() {
  ...
  const addToSlip = () => {
    ...
    // Check if number already in slip, if so update/merge
  ...
  const handleSubmitAll = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await placeMultiBets('Single Open', selectedMarket, betSlip);
      if (res.success) {
  ...
  return (
    <div id="single-open-arena" className="space-y-6 pb-12 max-w-4xl mx-auto">
      {/* Return button */}
      <button 
        onClick={() => navigate('/')}
      ...
      <div className="bg-gradient-to-r from-zinc-950 via-neutral-900 to-amber-950/10 border-2 border-amber-500/20 p-6 rounded-2xl relative overflow-hidden">
        ...
        <p className="text-neutral-405 text-xs mt-1.5 max-w-2xl leading-relaxed">
          Predict the opening digit of the published draw (range <strong className="text-amber-400 font-mono text-sm">0 - 9</strong>). Place multiple simultaneous predictions using our smart Betting Slip. Winning predictions award <strong className="text-amber-400 font-serif">9x coins</strong> instantly!
        </p>
      </div>
