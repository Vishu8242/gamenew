...
export default function SingleCloseGame() {
  ...
  // Betting slip for multiple bets
  ...
  // Default select first open market
  useEffect(() => {
  ...
  const addToSlip = () => {
    ...
    if (selectedNumber === '') {
      setError('Please select a digit first.');
      return;
    }

    if (betCoins <= 0) {
      setError('Enter a valid amount of coins.');
      return;
    }

    // Check if number already in slip
    const existingIndex = betSlip.findIndex(b => b.number === selectedNumber);
    if (existingIndex > -1) {
      const updated = [...betSlip];
      updated[existingIndex].coins += betCoins;
      setBetSlip(updated);
    } else {
      setBetSlip([...betSlip, { number: selectedNumber, coins: betCoins }]);
    }

    setSelectedNumber('');
    setSuccess(`Digit "${selectedNumber}" added to betting slip!`);
  };
  ...
  const handleSubmitAll = async (e: React.FormEvent) => {
    ...
    if (betSlip.length === 0) {
      setError('Your betting slip is empty. Add some bets above first.');
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await placeMultiBets('Single Close', selectedMarket, betSlip);
      if (res.success) {
        setSuccess(`⚜️ Bets placed successfully! Commited total of ${totalRequired} coins inside "${selectedMarket}". Best of luck! ⚜️`);
        setBetSlip([]);
      } else {
        setError(res.msg);
      }
  ...
  return (
    <div id="single-close-arena" className="space-y-6 pb-12 max-w-4xl mx-auto">
      {/* Return button */}
      <button 
        onClick={() => navigate('/')}
      ...
      {/* Header Info Card */}
      <div className="bg-gradient-to-r from-zinc-950 via-neutral-900 to-amber-951/10 border-2 border-amber-500/20 p-6 rounded-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 text-xs text-amber-400 font-mono uppercase tracking-widest font-bold mb-1">
          <Trophy className="w-4 h-4 text-amber-500 animate-pulse" /> Gold Payout Multiplier: 9x
        </div>
        ...
        <p className="text-neutral-405 text-xs mt-1.5 max-w-2xl leading-relaxed">
          Predict the closing digit of the published draw (range <strong className="text-amber-400 font-mono text-sm">0 - 9</strong>). Place multiple simultaneous predictions using our smart Betting Slip. Winning predictions award <strong className="text-amber-400 font-serif">9x coins</strong> instantly!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-zinc-950 border border-amber-500/25 rounded-2xl shadow-2xl p-5 sm:p-6 space-y-6">
          ...
          {/* Choose numbers */}
          <div>
            <label className="block text-[10px] font-mono text-amber-500/80 uppercase tracking-wider mb-2.5">
              2. Select Guessing Digit (0 - 9)
            </label>
              ...
              {['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].map((num) => {
                ...
                return (
                  <button
                    ...
                    className={`py-3 rounded-lg border font-serif text-lg font-black transition-all cursor-pointer ${
                      isSelected 
                        ? 'bg-gradient-to-r from-amber-600 to-yellow-400 text-zinc-950 border-amber-400 scale-105' 
                        : 'bg-zinc-900 border-amber-500/10 text-neutral-300 hover:border-amber-500/30 hover:text-amber-400'
                    }`}
                  >
                    {num}
                  </button>
          ...
          <button
            type="button"
            onClick={addToSlip}
            className="w-full py-3 bg-zinc-900 hover:bg-zinc-855 border border-amber-400 text-amber-400 font-mono font-black tracking-widest text-xs uppercase rounded-lg cursor-pointer flex items-center justify-center gap-1.5 active:scale-98 transition-transform"
          >
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-amber-500/10 pb-2">
              <h3 className="text-amber-400 text-xs font-mono font-bold tracking-widest uppercase">
                ⚜️ Active Betting Slip
              </h3>
              {betSlip.length > 0 && (
                <button 
                  onClick={clearSlip}
                  className="text-neutral-500 hover:text-red-400 font-mono text-[10px] transition-colors cursor-pointer border-none outline-none"
                >
                  Clear Slip
                </button>
              )}
            ...
            {betSlip.length === 0 ? (
              <div className="py-12 text-center text-neutral-600 font-mono text-xs">
                Your slip is empty.<br />Select a digit and add it to start.
              </div>
            ) : (
                ...
                {betSlip.map((item, idx) => (
                  <div key={idx} className="bg-zinc-900/60 border border-amber-500/5 p-2.5 rounded-lg flex items-center justify-between text-xs font-mono animate-fadeIn">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-neutral-500">DIGIT:</span>
          ...
          <div className="mt-6 border-t border-amber-500/10 pt-4 space-y-4">
            <div className="space-y-1.5 text-xs font-mono">
              ...
              <div className="flex justify-between text-neutral-400">
                ...
                <span className="text-emerald-400 font-bold font-serif">+{(totalCurrentCoins * 9).toLocaleString()} Coins</span>
              </div>
            </div>
